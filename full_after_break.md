...
候选人画像、结构化字段、能力雷达图、面试重点、可导出的评估报告。
...
通过字段抽取形成基本画像，通过雷达图和时间线形成判断，通过弱表达提示形成优化建议，最后输出评估报告或分享卡。
...
通过条款分块和关键字段抽取降低阅读成本，通过风险红绿灯和白话解释帮助判断，通过谈判清单形成可执行输出。
...
通过交易抽取形成结构化流水，通过分类和异常识别帮助判断，通过可视化和导出产物完成月度复盘或报税准备。
...
通过元数据抽取建立阅读入口，通过方法与结果摘要降低阅读成本，通过思维导图和术语卡片形成可复用知识结构。
